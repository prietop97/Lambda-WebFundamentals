...
let fullNames = [];
runners.forEach(value => fullNames.push(`${value.first_name} ${value.last_name}`))
//console.log(fullNames);      //ANSWER

// ==== Challenge 2: Use .map() ====
// The event director needs to have all the runners' first names in uppercase because the director BECAME DRUNK WITH POWER. Populate an array called `firstNamesAllCaps`. This array will contain just strings.
let firstNamesAllCaps = [];
firstNamesAllCaps = runners.map(value => value.first_name.toUpperCase());
//console.log(firstNamesAllCaps);   //ANSWER

// ==== Challenge 3: Use .filter() ====
// The large shirts won't be available for the event due to an ordering issue. We need a filtered version of the runners array, containing only those runners with large sized shirts so they can choose a different size. This will be an array of objects.
let runnersLargeSizeShirt = [];
runnersLargeSizeShirt = runners.filter(value => value.shirt_size === "L")
//console.log(runnersLargeSizeShirt);    //ANSWER
...
//console.log(ticketPriceTotal);     //ANSWER
...
// console.log(highDonation);   //ANSWER
// console.log(lowDonation);    //ANSWER
