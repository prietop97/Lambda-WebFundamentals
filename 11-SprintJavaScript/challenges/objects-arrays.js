...
// const universities = [];
universities = graduates.map(value=>value.university).sort();
...
let unisWithUni = [];
unisWithUni = graduates.filter(value => value.university.includes("Uni"))
...
lowCaseAnimalNames = zooAnimals.map(value=>value.animal_name.toLowerCase());
...
lowPopulationAnimals = zooAnimals.filter(value=>value.population<5);
